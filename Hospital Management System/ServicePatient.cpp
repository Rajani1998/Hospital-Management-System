...
ServicePatient::ServicePatient(string N,string G,string Ad,string Ph,int Ag,double B,string Ser):Patient(N,G,Ad,Ph,Ag,B){
	Service=Ser;
}
ServicePatient::~ServicePatient()
{
...
ostream& operator <<(ostream& output,ServicePatient &S){
	...
	output<<endl<<"Patient Blood Group "<<S.getBloodGroup();
